...
/***/ function(module, exports, __webpack_require__) {
	...
	//console.log(config.network_data);
	  // replace undersores with space in row/col names 用空格替换下划线
	  _.each(['row', 'col'], function (inst_rc) {
	    var inst_nodes = config.network_data[inst_rc + '_nodes'];
		  //console.log(inst_nodes);
        //inst_nodes=config.network_data[row_nodes];
        //inst_nodes=config.network_data[col_nodes];
	    var has_cats = check_nodes_for_categories(inst_nodes);//判断是否存在":" 若不存在 返回false
	    ...
	    inst_nodes.forEach(function (d) {
	      ...
	      d.name = d.name.replace(/_/g, ' ');//g表示全局模式 /_/g表示匹配字符串中所有"_"的实例
	    });
	  });

	  config.network_data.row_nodes_names = _.pluck(config.network_data.row_nodes, 'name');//_.pluck 提取一个集合里指定的属性值
	  config.network_data.col_nodes_names = _.pluck(config.network_data.col_nodes, 'name');//提取节点的名字

	  config.sim_mat = check_sim_mat(config);//检验行数是否等于列数


	  var filters = get_available_filters(config.network_data.views);//返回views中除了nodes的其它属性值 包括 N_row_sum N_row_var dist

	  var default_states = {};
		//mult_cats.json中为空 不执行下列代码
	  ...
	  // process view 将views中nodes的下划线替换为空格
	  if (_.has(config.network_data, 'views')) {//判断network_data中是否含有views对象
	    config.network_data.views.forEach(function (inst_view) {
	      ...
	      var inst_nodes = inst_view.nodes;//views中的节点

	      // proc row/col nodes names in views
	      _.each(['row', 'col'], function (inst_rc) {
			//check_nodes_for_categories 判断是否存在":" 若不存在 返回false
	        var has_cats = check_nodes_for_categories(inst_nodes[inst_rc + '_nodes']);//inst_node['row_nodes'] inst_node[col_nodes']
			//console.log(has_cats);//cats.json false
	        inst_nodes[inst_rc + '_nodes'].forEach(function (d) {

	          if (has_cats) {//cats 中返回false
	            d.name = d.name.split(super_string)[1];
	          }
	  ...
	  var row_nodes = config.network_data.row_nodes;
		...
		//console.log(config.network_data);
	  // add names and instantaneous positions to links 给links添加names和瞬时的位置
	  config.network_data.links.forEach(function (d) {
		//console.log(row_nodes[d.source].name+" "+ col_nodes[d.target].name);//rn 0   cn 1
	    d.name = row_nodes[d.source].name + '_' + col_nodes[d.target].name;//行作为源 列作为目标 行名_列名
		 // console.log(d.name);//rn 0_cn 1
	    d.row_name = row_nodes[d.source].name;
		  //console.log("row_name "+ d.row_name);//rn 0
	    d.col_name = col_nodes[d.target].name;
		 // console.log("col_name "+ d.col_name);// cn 1
	  });

	  // transpose network if necessary 如果需要变换network
		//console.log(config.transpose);//tranpose默认为false 所以默认不执行下列代码
	  if (config.transpose) {
	  ...
	  // initialize cluster ordering - both rows and columns 初始化聚类排序
	  config.inst_order = {};
		//执行else
...
/***/ function(module, exports) {
	...
	module.exports = function get_available_filters(views) {
	  ...
	  _.each(views, function (inst_view) {
	    var inst_keys = _.keys(inst_view);//取属性名
		  //console.log(inst_keys);

	    _.each(inst_keys, function (inst_key) {

	      if (inst_key != 'nodes') {

	        if (!_.has(filter_data, inst_key)) {//has 判断对象是否含有指定的属性名 如果filter_data中不包含inst_key属性名
	          filter_data[inst_key] = [];
	        }

	        filter_data[inst_key].push(inst_view[inst_key]);
			//console.log(filter_data);
	        filter_data[inst_key] = _.unique(filter_data[inst_key]);//去掉重复
	      }
	    });
	  });

	  var tmp_filters = _.keys(filter_data);//取属性名
		//console.log(filter_data);
	  _.each(tmp_filters, function (inst_filter) {

	    var options = filter_data[inst_filter];
		  //console.log(options+" "+options.length);
	    var num_options = options.length;//1
	  ...
	  filters.possible_filters = possible_filters;
		//console.log(possible_filters);//mult_cats.json例子中该值为空
	  filters.filter_data = filter_data;//filter_data保存的是views中除了nodes的属性值 包括N_row_sum N_row_var dist
...
/***/ function(module, exports) {
	...
	module.exports = function check_nodes_for_categories(nodes) {
	  ...
	  _.each(nodes, function (inst_node) {
		  //console.log(inst_node.name+" name");
		  //console.log(inst_node.name.indexOf(super_string)+" indexOf");//返回某个指定的字符串值在字符串中首次出现的位置。
	    if (inst_node.name.indexOf(super_string) < 0) {//小于0表示不存在该元素
	      has_cat = false;
	    }
	  });
